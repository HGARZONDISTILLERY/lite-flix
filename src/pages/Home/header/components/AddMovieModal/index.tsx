...
const AddMovieModal = ({ AddMovieButton }: AddMovieProps) => {
  ...
  useEffect(() => {
    if (
      movieElement && movieElement.title &&
  ...
  const addMovieContentForm = (
    <>
      ...
      <form onSubmit={handleSave}>
        ...
        />
        <Box>
          <MovieModalButton type="submit" sx={{background: '#fff', opacity: .9}}>
            Subir película
          </MovieModalButton>
        </Box>
      </form>
  ...
  const confirmationContentForm = (
    <>
      <Logo />
      <form>
        <Typography sx={{ marginTop: '72px', color: '#fff', fontWeight: '700', fontSize: '24px' }}>
          ¡Felicitaciones!
        </Typography>
    ...
    <div>
      ...
      >
        <Box sx={containerMovieModalStyle}>
          ...
          { !movieElementIsLoading && !movieElementUploaded && addMovieContentForm }
        </Box>
      </Modal>
    </div>
  )
